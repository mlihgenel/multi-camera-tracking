...
from src.workers.camera_worker import camera_worker 
from src.core.log import logger
from src.detections.track import yolo_track_frame
from ultralytics import YOLO
import os 
from src.utils.save_track_moment import TrackVideoWriter
from dotenv import load_dotenv 
from src.core.fps_count import FPScounter
...
def main():
    ...
    try:
        MODEL_TYPE = os.getenv("MODEL_PATH").split("/")[1] # models/yolo11n.pt -> yolo11n.pt
        model = YOLO(MODEL_TYPE)
        tracker = os.getenv("TRACKER_CONFIG").split("/")[1] # models/botsort.yaml -> botsort
        logger.info(f"YOLO modeli oluşturuldu: {MODEL_TYPE} / tracker: {tracker.split(".")[0]}")
    ...
    try:
        ...
        logger.error(f"FPS hesaplama nesnesi oluşturulamadı: {e}")
    
    try:
        db = TrackerDB()
    except Exception as e:
        logger.error(f"Database oluşturulken hata oldu: {e}")
    
    try:
        while True:

            cam_id, frame_id, timestamp, frame, motion = result_queue.get()
            if frame is None:
                try:
            ...
            if motion and not prev_motion[cam_id]: 
                ...
                try:
                    db.add_motion(cam_id, frame_id, motion=1, timestamp=timestamp)
                except Exception as e:
                    logger.error(f"Motion tablosuna eklenemedi: {e}")
                ...
                try:
                    db.add_motion(cam_id, frame_id, motion=0, timestamp=timestamp)
                except Exception as e:
                    logger.error(f"Motion tablosuna eklenemedi: {e}")
            ...
            if motion and not video_writers[cam_id].active:
                video_writers[cam_id].start(cam_id)
            ...
            cv2.putText(frame, "FPS: {:.2f}".format(fps), (10, 350), cv2.FONT_HERSHEY_PLAIN, 1.5, (0, 255, 0), 2)           
